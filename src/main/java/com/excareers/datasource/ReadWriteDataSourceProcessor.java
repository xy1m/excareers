package com.excareers.datasource;
...
public class ReadWriteDataSourceProcessor implements BeanPostProcessor {
    ...
    private boolean              defaultMethodRead        = false;

    private Map<String, Boolean> readMethodMap            = new HashMap<String, Boolean>();

    private Map<String, String>  methodMatchMap           = new ConcurrentHashMap<String, String>();
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName)
        ...
        try {
            ...
            if (readMethodMap.containsKey("*")) {
                defaultMethodRead = true;
            }
            // 去除*带来的干扰，* 默认访问写库
            readMethodMap.remove("*");
        } catch (Exception e) {
    ...
    public Object determineReadOrWriteDB(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (isChoiceReadDB(pjp.getSignature().getName())) {
            log.debug("read only = " + pjp.getSignature().getName());
            ReadWriteDataSourceDecision.markRead();
        } else {
            log.debug("write = " + pjp.getSignature().getName());
    ...
    private boolean isChoiceReadDB(String methodName) {
        ...
        String bestNameMatch = methodMatchMap.get(methodName);
        if (null == bestNameMatch) {
            ...
            methodMatchMap.put(methodName, (null == bestNameMatch ? methodName : bestNameMatch));
        }
